...
- 획 그리는 애니메이션(0.95초)과 100% 완벽 일치(0.95s 동기화)
- 붓이 지면에 닿아 획을 긋는 순간부터 뗄 때까지 사각사각 화선지 결 마찰음 일치
- 자극적이지 않고 편안하며 은은한 자연 힐링 볼륨 레벨
"""
import os
import sys
import wave
...
def create_calm_natural_brush_sfx(style_idx: int, duration: float = 0.95):
    """
    획 애니메이션(0.95초)과 1:1 완벽 동기화되는 차분하고 자연스러운 서예 붓글씨 마찰음 합성
    """
    num_samples = int(duration * SAMPLE_RATE)
    t = np.linspace(0, duration, num_samples, endpoint=False)
    
    # 1. 붓 착지 틱 (부드러운 붓끝 안착감 - Soft Tactile Touchdown: 0.0 ~ 0.05s)
...
def generate_all_5_brush_sfx(out_dir="assets/audio/sfx"):
    ...
    sfx_info = [
        ("01_soft_hanji_glide.wav", 1, 0.95, "은은한 한지 묵향 붓터치 (부드럽고 온화함)"),
        ("02_crisp_mulberry_scratch.wav", 2, 0.95, "차분한 닥종이 사각사각 결 ASMR (은은하고 자연스러움)"),
        ("03_bold_ink_strike.wav", 3, 0.95, "묵직하고 고요한 일필 (깊이 있는 서예 호흡)"),
        ("04_delicate_tip_flick.wav", 4, 0.95, "유려한 붓끝 곡선 흐름 (우아한 여운)"),
        ("05_zen_bamboo_brush.wav", 5, 0.95, "다도 속 정갈한 먹물 터치 (조용하고 맑은 터치)")
    ]
    
    print("🖌️ 0.95초 획 동기화 실사 서예 ASMR 5종 합성 시작...\n")
    ...
    audio_int16 = create_calm_natural_brush_sfx(2, duration=0.95)
